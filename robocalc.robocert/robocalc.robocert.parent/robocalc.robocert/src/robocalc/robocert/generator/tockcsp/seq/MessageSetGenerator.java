...
public class MessageSetGenerator {
	 ...
	 * @param tgg target group generator, used for resolving a target's universe
	 *            field.
	 * @param mso optimiser for message sets, used for named message set generation.
	 * @param msg generator for message specs.
	 */
	@Inject
	public MessageSetGenerator(CSPStructureGenerator csp, TargetGroupGenerator tgg, MessageSetOptimiser mso,
	 ...
	 * @param registrar function that receives the optimised set before generation,
	 *                  and which should re-attach the new set to the object graph
	 *                  in place of m (eg, if m is <code>foo.getSet()</code>,
	 *                  registrar should be <code>foo::setSet</code>).
	 *
	 ...
	 *
	 * @apiNote This generator does not optimise sets before generating them; it
	 *          just emits the direct CSP equivalent of the set definition.
	 *
	 * @param m the set in question.
	 *
	...
	private CharSequence generateOp(BinarySetOperator op) {
		...
		default -> throw new IllegalArgumentException("unexpected binary set operator: %s".formatted(op));
		};
	}
