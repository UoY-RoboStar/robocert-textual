...
public class InteractionGenerator {
  ...
  private CharSequence generateWithoutMemory(Interaction s) {
    final var ctx = lcf.context(s);
    final var lines = lcf.actors(ctx);
    ...
    return switch (lines.size()) {
      ...
      default -> generateMultiLifeline(s, ctx, lines);
    };
  }
  ...
  private CharSequence generateMultiLifeline(Interaction s, InteractionContext ctx,
      List<ActorContext> lines) {
    final var cs = csp.sets();

    final var untilChans = ctx.untilChannelIfNeeded()
        .map(u -> csp.enumeratedSet(TERM_CHANNEL, u));
    final var ctrlChans = untilChans.orElse(cs.set(TERM_CHANNEL));

    final var alphas = defs(lines, ActorContext::alphaCSP, x -> alpha(s, x, ctrlChans));

    final var term = csp.pre(TERM_CHANNEL, csp.skip());
    final var procs = defs(lines, ActorContext::procCSP,
        x -> cs.tuple(csp.seq(sg.generate(s.getFragments(), x), term)));
    ...
    final var body = untilChans.map(u ->
        let.and(untilGen.process(ctx))
    ...
    // RoboChart ones).
    return csp.bins().hide(cs.tuple(body), ctrlChans);
  }
  ...
  private CharSequence alpha(Interaction s, ActorContext ctx, CharSequence ctrlSet) {
    ...
    final var sets = StreamHelper.push(ctrlSet, msgSets).map(CharSequence::toString)
