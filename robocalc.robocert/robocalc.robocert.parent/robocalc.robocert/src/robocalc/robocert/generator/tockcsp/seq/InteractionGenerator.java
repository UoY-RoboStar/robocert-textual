...
public class InteractionGenerator {
  ...
  private CharSequence generateMultiLifeline(InteractionContext s,
    ...
    final var untilChans = s.untilChannelIfNeeded()
        .map(u -> csp.enumeratedSet(TERM_CHANNEL, u));
    final var ctrlChans = untilChans.orElse(cs.set(TERM_CHANNEL));

    final var alphas = defs(lines, ActorContext::alphaCSP, x -> alpha(x, ctrlChans));
  ...
  private CharSequence alpha(ActorContext ctx, CharSequence ctrlSet) {
