 ...
 * @author Matt Windsor
 */
public class ActorContextFinder {
	 ...
	 */
	@Inject
	public ActorContextFinder(DefinitionHelper dh) {
		this.dh = dh;
	}
	 ...
	 *
	 * @apiNote An empty-optional result (not an empty stream) denotes the special
	 *          case of a system module actor. These do not report any contexts: any
	 *          sequence diagram using a system module actor only has communications
	 *          between it and a context actor, and we can get the contexts
	 *          unambiguously from just the latter.
	 *
	 * @param a the actor in question.
	 *
	 * @return a stream of contexts containing operations, events, and variables
	 *         visible to the given actor.
	 */
	public Optional<Stream<Context>> contexts(Actor a) {
		...
		if (a instanceof ContextActor x)
			return Optional.of(contextsOfTargetContext(x.getGroup().getTarget()));

		// See API note above.
		if (a instanceof SystemModuleActor s)
	...
	/**
	 * Retrieves RoboChart contexts deriving from a {@link ComponentActor} attached
	...
	/**
	 * Retrieves RoboChart contexts deriving from a {@link ContextActor} attached to
	 * the given target.
	 *
	 ...
	 * @return the stream of contexts in scope of the actor.
	 */
	private Stream<Context> contextsOfTargetContext(Target t) {
