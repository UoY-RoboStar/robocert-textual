...
public class MessageValidator extends AbstractDeclarativeValidator {
    ...
    public void checkEventTopicHasConnection(Message m) {
        ...
        final var candidates = eventRes.resolve(e, from, to).collect(Collectors.toUnmodifiableSet());
        ...
        if (candidates.isEmpty()) {
            if (!m.isOutbound()) {
                ...
                error("Event topic is not outbound and does not correspond to any connection", Literals.MESSAGE__TOPIC, EVENT_TOPIC_HAS_CONNECTION);
            }
            return;
        }

        if (1 < candidates.size()) {
            error("Event topic corresponds to too many connections", Literals.MESSAGE__TOPIC, EVENT_TOPIC_HAS_CONNECTION);
        }
        // TODO(@MattWindsor91):
    }
    ...
    public void checkEdgeFlow(Message m) {
        if (EcoreUtil.equals(m.getFrom(), m.getTo())) {
            error("The 'from' of a message must not be equal to the 'to' of the message", Literals.MESSAGE__FROM, EDGE_ACTORS_INDISTINCT);
        }
    }
    ...
    public void checkMessageOperationFlow(Message m) {
        ...
        if (m.getFrom().isWorld()) {
            error("Operation messages must not originate from a context", Literals.MESSAGE__FROM, OPERATION_FROM_WORLD);
        }
        if (!m.getTo().isWorld()) {
            error("The 'to' of a message with an operation topic must be a world", Literals.MESSAGE__TO, OPERATION_NOT_TO_WORLD);
        }
    ...
    public void checkArgumentsAgainstParameters(Message m) {
        ...
        if (nparams != nargs) {
            error("The arguments of a message must have exactly as many elements as its topic has parameters.", Literals.MESSAGE__ARGUMENTS, HAS_CORRECT_ARGUMENT_COUNT);
        }
        ...
        for (var i = 0; i < safeRange; i++) {
            if (!argumentTypeOk(args.get(i), params.get(i))) {
                error("The arguments of a message must be type-compatible with their corresponding parameters.", Literals.MESSAGE__ARGUMENTS, ARGUMENTS_TYPE_COMPATIBLE);
            }
        }
    }
